...
public class SentryFakePlayer {
    ...
    private static RobustFakePlayer createRobustFakePlayer(ServerLevel level, String name) {
        ...
        if (fp.connection == null) {
            try {
                ...
                fp.connection = new net.minecraft.server.network.ServerGamePacketListenerImpl(level.getServer(), fakeConnection, fp) {
                    @Override
    ...
    public static void sync(FakePlayer fp, SentryArmBlockEntity arm, float yaw, float pitch, ItemStack gunStack) {
        ...
        if (needsSwitch) {
            ...
            if (gunStack.hasTag()) {
                if (!java.util.Objects.equals(currentFakeItem.getTag(), gunStack.getTag())) {
                    currentFakeItem.setTag(gunStack.getTag().copy());
                }
            } else {
                if (currentFakeItem.hasTag()) currentFakeItem.setTag(null);
            }
